...
**Delete everything in the Quarto document below the YAML header.** 
It's just example text—we're not attached to it!
...
If you want to run line by line in the code chunk, you can place the text cursor on the line you want to run and press `CMD+ENTER/CTRL+ENTER`. The current line is run and the text cursor is moved to the next line.
Just like in the .R scripts that we've been using in past weeks.
:::
...
It may be shown in the Viewer pane (like in the picture below) or in a new browser window.
There are a few ways you can set this option to whichever version you prefer.
Follow the instructions that work for you—they all do the same thing.
...
Create a new second-level header in your Quarto document called `## Making plots`.
Everything else in this tutorial will go into this section.
...
Showing the data itself requires a further step, which we'll turn to next.
...
In fact, Grammar of Graphics is where you get the GG in ggplot!
:::
...
To make ggplot easy for users of different Englishes, it's possible to write the colour aesthetic either as the British-style `colour` or the American-style `color`!
