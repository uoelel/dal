...
Note that the *y*-axis label is still "count" but it is in fact proportions. You can change the labels of the axes and the fill legend with `labs()`.
...
Notice how `colour = gender` must be inside the `aes()` function, because we are trying to map `colour` to the values of the column `gender`.
